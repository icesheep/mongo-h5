...
class Ready extends Component {
  ...
  render() {
    ...
    const {begin_time, end_time, banner_images, banner_playurl, banner_title, data_list, msg, desc} = this.props;
    ...
    return (
      <div className={styles.main}>
        <div className={styles.div1} style={{backgroundImage: `url(${banner_images})`}}>
          ...
          <div className={styles.item3}>
            <div className={styles.bg1}>
              <div className={styles.item3_1}>距离开始仅剩</div>
              <div className={styles.item3_2}>
                {hour.split('').map(v => <div className={styles.time}>{v}</div>)}
                <div className={styles.devide}>:</div>
                {minute.split('').map(v => <div className={styles.time}>{v}</div>)}
                <div className={styles.devide}>:</div>
                {second.split('').map(v => <div className={styles.time}>{v}</div>)}
              </div>
            </div>
          </div>
        </div> 
        <div className={styles.div2}>
          ...
          {data_list.length>0&&data_list.map((v, index) => 
            <div>
              <div style={{backgroundImage: `url(${v.images})`}} className={styles.item6}> 
                <img onClick={()=>this.playLastAudio(v, index)} src={index === playing ? PauseImg : PlayImg} />
        ...
        ></audio> */}
        <VideoJsForReact
          sourceChanged={(player,players) => {this.setState({players})}}
          onReady={(player,players) => {this.setState({players})}}
          onEnded={this.playLastAudio}
          {...videoJsOptions}
        />
      </div>
