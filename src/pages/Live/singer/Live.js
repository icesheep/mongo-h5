  ...
  render() {
    ...
    return (
      <div className={styles.main}>
        <div className={styles.mask} style={{background: playing ? 'rgba(0,0,0,.6)' : 'rgba(0,0,0,.4)'}}></div>
        <div className={styles.div1} style={{backgroundImage: banner_images&&`url(${banner_images})`}}>
          ...
          <div className={styles.img2} style={{display: playing? '':'none'}}>
            <Wave 
              ...
              height={document.getElementsByTagName('html')[0].style.fontSize.replace('px','')*1.2} />
          </div>
        </div> 
        ...
        {/* 直播音频播放控件 */}
        <VideoJsForReact
          sourceChanged={(player,players) => {this.setState({players},this.play)}}
          onReady={(player,players) => {this.setState({players},this.play)}}
          {...videoJsOptions}
        />
      </div>
    );
